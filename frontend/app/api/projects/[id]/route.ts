...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Only update DB if values actually changed, without locking on read
    if (project.survival_score !== risk.survivalScore || project.risk_level !== risk.riskLevel || project.predicted_delay_hours !== risk.predictedDelayHours) {
      try {
        db.prepare('UPDATE projects SET survival_score = ?, risk_level = ?, predicted_delay_hours = ? WHERE id = ?')
          .run(risk.survivalScore, risk.riskLevel, risk.predictedDelayHours, id);
      } catch {}
    }

    // Pre-calculate nextTask to avoid duplicate API calls
    const completedIds = new Set((tasks as any[]).filter(t => t.status === 'completed' || t.status === 'skipped').map(t => t.id));
    ...
    const eligibleTasks = (tasks as any[])
    ...
    return NextResponse.json({
      ...
      risk,
      nextTask: { nextTask: eligibleTasks[0] || null, queue: eligibleTasks.slice(1, 4) },
    });
  } catch (err) {

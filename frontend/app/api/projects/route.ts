...
export async function POST(req: NextRequest) {
  try {
    ...
    const projectDeadline = (deadline && typeof deadline === 'string') ? deadline.trim() : '';
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'active', ?, 100)
    `).run(projectId, userId, goal.slice(0, 100), description || goal, goal, projectDeadline, estimatedEffortHours || 20);

    // Generate tasks with AI
    const { tasks, dependencies } = await generateTasks(goal, projectDeadline, estimatedEffortHours || 20);

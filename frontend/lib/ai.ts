...
export async function parseGoal(text: string): Promise<ParsedGoal> {
  ...
  const prompt = `You are an AI goal parser. Parse this user input and extract goal title, priority, estimated effort, and deadline ONLY IF explicitly stated.
...
\`\`\`json
{
  ...
  "deadline": "YYYY-MM-DD format ONLY if the user explicitly stated a deadline in the input (e.g. 'in 3 days', 'by Friday', 'due on Oct 10'). If user did NOT mention any deadline, return empty string \"\"",
  "priority": "critical|high|medium|low",
...
export function calculateRisk(
  ...
  if (!deadlineDate) {
    return {
      ...
      failureReason: '',
      recommendation: 'No deadline set. Work at your own pace.',
    };
  }
